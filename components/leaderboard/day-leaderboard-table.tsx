...
interface DayLeaderboardTableProps {
...
export function DayLeaderboardTable({ entries, date, onAssetClick }: DayLeaderboardTableProps) {
  if (entries.length === 0) {
    return <div className="text-center py-12 text-gray-500">No submissions for this day.</div>
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 px-4 sm:px-6 py-3 border-b bg-gray-50 text-xs font-medium text-gray-500 uppercase tracking-wide">
        ...
        <div className="w-20 text-center shrink-0">Total</div>
      </div>
      ...
      <div className="divide-y">
        {entries.map((entry) => {
          ...
          const isUnrated = entry.avg_total_score === 0 && (entry.static_count || 0) + (entry.video_count || 0) > 0
          ...
          return (
            ...
            >
              ...
              <div className="w-20 text-center shrink-0">
                {isUnrated ? (
                  <Badge variant="secondary" className="text-xs">Unrated</Badge>
                ) : (
